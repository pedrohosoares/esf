...
if ( defined( 'FW' ) ) {
	...
	if ( $header_image == '' ) {
		...
		$bg_image = $header_image != '' ? 'style="background-image: url(' . $header_image[ 'url' ] . ')"' : '';
	}
	if(is_category()){
		if (function_exists('get_wp_term_image'))
		{
			$category = get_queried_object();
			$bg_image = get_wp_term_image($category->term_id); 
			$bg_image = !empty($bg_image)?'style="background-image: url('.$bg_image.')"':'';
		}else{
			$bg_image = $page_global_image != '' ? 'style="background-image: url(' . $page_global_image[ 'url' ] . ')"' : '';
		}
	}
