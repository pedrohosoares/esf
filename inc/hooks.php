...
if ( !function_exists( 'geobin_widget_init' ) ) {
	...
	function geobin_widget_init() {
		if ( function_exists( 'register_sidebar' ) ) {
			...
			register_sidebar(
			array(
				'name'			 => esc_html__( 'Category Page', 'geobin' ),
				'id'			 => 'sidebar-category',
				'description'	 => esc_html__( 'Category', 'geobin' ),
				'before_widget'	 => '<div id="%1$s" class="widget %2$s">',
				'after_widget'	 => '</div> <!-- end widget -->',
				'before_title'	 => '<h3 class="widget-title">',
				'after_title'	 => '</h3><span class="animate-border border-offwhite tw-mt-20"></span>',
			...
			register_sidebar(
			array(
				'name'			 => esc_html__( 'Tag Page', 'geobin' ),
				'id'			 => 'sidebar-tag',
				'description'	 => esc_html__( 'Tag', 'geobin' ),
				'before_widget'	 => '<div id="%1$s" class="widget %2$s">',
				'after_widget'	 => '</div> <!-- end widget -->',
				'before_title'	 => '<h3 class="widget-title">',
				'after_title'	 => '</h3><span class="animate-border border-offwhite tw-mt-20"></span>',
...
function geobin_content_read_more( $num = 20 ) {
	...
	echo geobin_kses( $trimmed_content );
	echo '</div><div class="post-footer"><a href="' . get_the_permalink() . '" class="ts-readmore">' . esc_html__( 'Continue lendo', 'geobin' ) . '<i class="icon icon-arrow-right"></i></a>';
}
...
function geobin_search_form( $form ) {
    ...
    <div class="search-widget input-group">
        <form method="get" action="' . esc_url( home_url( '/' ) ) . '" id="search">
                <input type="text" name="s" class="form-control"  placeholder="' . esc_attr__( 'Buscar..', 'geobin' ) . '" value="' . get_search_query() . '">
...
#add_action( 'admin_menu', 'geobin_remove_theme_settings', 999 );
