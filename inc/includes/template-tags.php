		...
		if ( get_post_type() === 'post' ) {
			...
			if( is_single() ){
				...
				if($dateModified != $date){
					echo '<span class="single-post-meta-date">';
					echo '|&nbsp;&nbsp;&nbsp; Atualizado em '. $dateModified;
					echo  '</span>';
				}
			...
			*/
			...
			if ( $category ) {
				echo '<span class="meta-categories post-cat"> <i class="icon icon-folder"></i>' . $category[0]->cat_name . ' </span>';
			}

			// Comments link.
			if ( comments_open() ) :
				echo '<span class="post-comment"><i class="icon icon-comment"></i> ';
				comments_popup_link( esc_html__( '0', 'geobin' ), esc_html__( '1 Comentário', 'geobin' ), esc_html__( '% Comments', 'geobin' ) );
				echo '</span>';
			endif;
			/*
			if ( !is_single() ) {
		...
		function geobin_xs_comment_style( $comment, $args, $depth ) {
			...
			?> id="comment-<?php comment_ID() ?>"><?php if ( 'div' != $args[ 'style' ] ) { ?>
				...
				function geobin_link_pages() {
					$args = array(
						'before'			 => '<div class="page-links"><span class="page-link-text">' . esc_html__( 'Mais páginas: ', 'geobin' ) . '</span>',
						'after'				 => '</div>',
						'link_before'		 => '<span class="page-link">',
						'link_after'		 => '</span>',
						'next_or_number'	 => 'next',
						'separator'			 => ' | ',
						'nextpagelink'		 => esc_html__( 'Próximo ', 'geobin' ) . '<I class="fa fa-angle-right"></i>',
						'previouspagelink'	 => '<I class="fa fa-angle-left"></i>' . esc_html__( ' Anterior', 'geobin' ),
					);
					wp_link_pages( $args );
				}
